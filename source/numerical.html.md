...
以前から気になっていた４次元$\mathbb{Z}_2$格子ゲージ理論の相転移の問題の数値計算をAIの力を借りてやってみた。昔の論文[Experiments with a Gauge-Invariant Ising System](https://doi.org/10.1103/PhysRevLett.42.1390) のFIG.3のようなプロットを出す問題。この論文より$\beta$の刻みを1/10程度にして相転移の際を見て、格子を$32^4$にし、熱化に時間がかかるのでsweepの時間を約10倍にした。結果エネルギーに飛びがあって、完全に一次相転移であることが納得できた。GPUも使えるので、さほど時間もかからない。良い時代になったものである。

![Z2格子ゲージ理論の相転移付近のエネルギー](z2latticegaugetheory.svg)

### コードへのリンク

上と同様のGoogle Colabへのリンク。

- [Z2格子ゲージ理論の相転移付近のエネルギーの計算](https://colab.research.google.com/drive/13EkIDT9cDXceliiBcEkN3EnIXdgCb2Vh?usp=sharing)

### 気がついたことのメモ

元論文のやり方は、スタートの配位として

- 空間を半分に分け、片方を完全に無秩序な配位、もう片方を完全に秩序な配位

としてメトロポリス法で配位を生成させていく。ここでも、このやり方を踏襲してやったのだが、体積が大きくなると熱化に時間がかかってしまう。

なんとか工夫しようとして、スタートの配位を

- 空間を４等分して、完全に無秩序な配位と完全に秩序な配位を互い違いにする

というものを採用したら熱化にかかる時間が減るだろうと（浅はかにも）思ってやってみた。結果、この場合には「間違った相」の方に行ってしまうことが（なぜかは分からないが）あった。一次相転移の場合、一旦間違った方に行ってしまうと正しい方に帰ってくるのに時間がかかってしまう。きっと元論文の著者の方々も知っていて（あるいはいろいろやってみて）、この「２等分の方法」をしたのだろうと思う。当時は今ほど気軽に数値計算することができなかったであろうことを想像すると、著者の方々の努力と見識に頭が下がる思いである。